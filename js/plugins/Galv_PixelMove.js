...
//  Galv_PixelMove.js
//-----------------------------------------------------------------------------
//  2025-05-11 - Version 1.5 - fixed issue with directional passabile tiles
...
//-----------------------------------------------------------------------------
/*:
 * @plugindesc (v.1.5) Gives the illusion of pixel movement for the player.
 * 
...
Game_Player.prototype.canPass = function(x, y, d, qx, qy) {
	x = Math.round(x);
	y = Math.round(y);
	let canPassTile = Game_CharacterBase.prototype.canPass.call(this,x,y,d);
	if (this._normMove) return canPassTile;

	// SLIGHT ISSUE: 1 quadrant offset when walking around directional passability tiles, might have to do with the diagonal movement on certain directions?
	...
	switch (d) {
		case 6: // RIGHT
			...
			/* CODE FOR 4 DIR BLOCKING TILE */
			} else if (qx === 2) {
				// If ? third of the tile
				if (qy == 2) {
					var pass = Game_CharacterBase.prototype.canPass.call(this,x,y - 1,2);
					if (!pass) this._diagSlide = Galv.PMOVE.getHorzVertDirs(3);
					return pass;
				}
				if (qy == 1) {
					var pass = Game_CharacterBase.prototype.canPass.call(this,x,y + 1,8);
					if (!pass) this._diagSlide = Galv.PMOVE.getHorzVertDirs(9);
					return pass;
				}
			/* END CODE FOR 4 DIR BLOCKING TILE */
		...
		case 8: // UP
			...
			/* CODE FOR 4 DIR BLOCKING TILE */
				...
				// If on lower third of the tile
				if (qx == 2) {
					var pass = Game_CharacterBase.prototype.canPass.call(this,x - 1,y,6);
					if (!pass) this._diagSlide = Galv.PMOVE.getHorzVertDirs(9);
					return pass;
				}
				if (qx == 1) {
					var pass = Game_CharacterBase.prototype.canPass.call(this,x + 1,y,4);
					if (!pass) this._diagSlide = Galv.PMOVE.getHorzVertDirs(7);
					return pass;
				}
			/* END CODE FOR 4 DIR BLOCKING TILE */
